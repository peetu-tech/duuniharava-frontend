        ...
        <div className="grid w-full gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-16 xl:gap-20">
          <section className="max-w-2xl space-y-6 sm:space-y-8 lg:space-y-10">
            ...
            <div className="space-y-4 sm:space-y-5">
              <h1 className="max-w-xl text-[2.3rem] font-black leading-[0.98] tracking-tight text-white sm:text-5xl lg:text-6xl xl:text-7xl">
                Luo tili ja rakenna työnhaku <span className="text-[#00BFA6]">yhteen työtilaan.</span>
              </h1>
              <p className="max-w-xl text-[15px] leading-7 text-gray-300 sm:text-lg sm:leading-9">
            ...
            <div className="grid gap-3.5 sm:gap-5">
              {signupHighlights.map((item, index) => (
                <div
                  key={item.title}
                  className="rounded-[26px] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-md transition-all duration-300 hover:border-[#00BFA6]/35 hover:bg-white/[0.06] sm:rounded-[28px] sm:p-6"
                >
                  <div className="flex items-start gap-4">
                    <span className="mt-1 flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#FF6F3C]/12 text-sm font-black text-[#FF6F3C]">
                      0{index + 1}
                    </span>
          ...
          <section className="rounded-[30px] border border-white/10 bg-white/[0.035] p-5 shadow-[0_20px_70px_rgba(0,0,0,0.35)] backdrop-blur-xl transition-all hover:border-white/20 sm:rounded-[34px] sm:p-8 lg:p-10">
            ...
            <form className="space-y-6 sm:space-y-7" onSubmit={onSubmit}>
              <div className="space-y-3">
                <label className="ml-1 block text-sm font-bold text-gray-300">Nimi</label>
                <input
                  type="text"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  className="w-full rounded-[22px] border border-white/10 bg-[#0F0F0F] px-5 py-4.5 text-base text-white outline-none transition-all placeholder:text-gray-600 focus:border-[#00BFA6]/50 focus:bg-white/[0.02] sm:rounded-[24px] sm:px-6 sm:py-5"
              ...
              <div className="space-y-3">
                <label className="ml-1 block text-sm font-bold text-gray-300">Sähköposti</label>
                <input
                  type="email"
                  value={email}
              ...
              <div className="space-y-3">
                <label className="ml-1 block text-sm font-bold text-gray-300">Salasana</label>
                <input
                  type="password"
                  value={password}
              ...
              <div className="pt-1 sm:pt-2">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full rounded-[22px] bg-gradient-to-r from-[#00BFA6] to-[#FF6F3C] px-5 py-4.5 text-base font-black text-black shadow-[0_0_20px_rgba(0,191,166,0.3)] transition-transform hover:scale-[1.01] active:scale-95 disabled:opacity-50 sm:rounded-[24px] sm:px-6 sm:py-5"
                >
            ...
            {error && (
              <div className="mt-6 rounded-[22px] border border-red-900/50 bg-red-500/10 p-4 text-center text-sm font-bold text-red-300 backdrop-blur-md sm:rounded-[24px]">
                {error}
              </div>
            )}

            <div className="mt-7 rounded-[22px] border border-white/8 bg-black/20 p-4 text-center sm:mt-8 sm:rounded-[24px] sm:p-5">
            ...
            <p className="mt-6 text-center text-[11px] font-medium uppercase tracking-[0.18em] text-gray-600 sm:text-xs">
